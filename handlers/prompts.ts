import { Server } from "@modelcontextprotocol/sdk/server/index.js";
import {
  ListPromptsRequestSchema,
  GetPromptRequestSchema,
} from "@modelcontextprotocol/sdk/types.js";
import { readFileSync } from "fs";
import { fileURLToPath } from "url";
import { dirname, join } from "path";

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

export enum PromptName {
  SIMPLE = "simple_prompt",
  COMPLEX = "complex_prompt",
  RESOURCE = "resource_prompt",
  VALIDATE_COMMUNITY_SERVER = "validate_community_server",
  VALIDATE_OFFICIAL_SERVER = "validate_official_server",
}

// Example completion values for prompt arguments
const EXAMPLE_COMPLETIONS = {
  style: ["casual", "formal", "technical", "friendly"],
  temperature: ["0", "0.5", "0.7", "1.0"],
  resourceId: ["1", "2", "3", "4", "5"],
};

// Sample tiny image for demonstration (MCP logo as base64)
const MCP_TINY_IMAGE =
  "";

/**
 * Helper function to load a markdown template file and replace placeholders
 * @param filename - Name of the markdown file (without path)
 * @param args - Object containing argument values to replace placeholders
 * @returns Processed markdown content with placeholders replaced
 */
function loadPromptTemplate(filename: string, args: Record<string, string>): string {
  // Read markdown file from prompts directory (one level up from handlers)
  const promptPath = join(__dirname, "..", "prompts", filename);
  let content = readFileSync(promptPath, "utf-8");

  // Replace all {{placeholder}} patterns with actual values
  for (const [key, value] of Object.entries(args)) {
    const placeholder = `{{${key}}}`;
    content = content.replace(new RegExp(placeholder.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), value);
  }

  return content;
}

export function setupPromptHandlers(server: Server) {
  server.setRequestHandler(ListPromptsRequestSchema, async () => {
    return {
      prompts: [
        {
          name: PromptName.SIMPLE,
          description: "A simple prompt without arguments",
        },
        {
          name: PromptName.COMPLEX,
          description: "A prompt with arguments for customization",
          arguments: [
            {
              name: "temperature",
              description: "Temperature setting (0-1.0)",
              required: true,
            },
            {
              name: "style",
              description: "Output style preference",
              required: false,
            },
          ],
        },
        {
          name: PromptName.RESOURCE,
          description: "A prompt that includes an embedded resource reference",
          arguments: [
            {
              name: "resourceId",
              description: "Resource ID to include (1-100)",
              required: true,
            },
          ],
        },
        {
          name: PromptName.VALIDATE_COMMUNITY_SERVER,
          description: "Validate a single community MCP server PR submission",
          arguments: [
            {
              name: "pr_url",
              description: "GitHub PR URL (e.g., https://github.com/owner/repo/pull/123)",
              required: true,
            },
          ],
        },
        {
          name: PromptName.VALIDATE_OFFICIAL_SERVER,
          description: "Validate a single official MCP server PR submission",
          arguments: [
            {
              name: "pr_url",
              description: "GitHub PR URL (e.g., https://github.com/owner/repo/pull/123)",
              required: true,
            },
          ],
        },
      ],
    };
  });

  server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    const { name, arguments: args } = request.params;

    if (name === PromptName.SIMPLE) {
      return {
        messages: [
          {
            role: "user",
            content: {
              type: "text",
              text: "This is a simple prompt without arguments. It provides a basic template for getting started with prompts.",
            },
          },
        ],
      };
    }

    if (name === PromptName.COMPLEX) {
      const temperature = args?.temperature || "0.7";
      const style = args?.style || "casual";

      return {
        messages: [
          {
            role: "user",
            content: {
              type: "text",
              text: `This is a complex prompt with customizable arguments:\n\nTemperature: ${temperature}\nStyle: ${style}\n\nYou can use these parameters to customize the behavior of your prompts.`,
            },
          },
          {
            role: "assistant",
            content: {
              type: "text",
              text: `I understand. You've provided a complex prompt with temperature=${temperature} and style=${style}. How would you like me to proceed?`,
            },
          },
          {
            role: "user",
            content: {
              type: "image",
              data: MCP_TINY_IMAGE,
              mimeType: "image/png",
            },
          },
        ],
      };
    }

    if (name === PromptName.RESOURCE) {
      const resourceId = parseInt(args?.resourceId as string, 10);

      if (isNaN(resourceId) || resourceId < 1 || resourceId > 100) {
        throw new Error(
          `Invalid resourceId: ${args?.resourceId}. Must be a number between 1 and 100.`
        );
      }

      // Create a reference to an existing resource
      const resourceUri = `test://static/resource/${resourceId}`;

      return {
        messages: [
          {
            role: "user",
            content: {
              type: "text",
              text: `This prompt includes Resource ${resourceId}. Please analyze the following resource:`,
            },
          },
          {
            role: "user",
            content: {
              type: "resource",
              resource: {
                uri: resourceUri,
                name: `Resource ${resourceId}`,
                mimeType: "text/plain",
                text: `Resource ${resourceId}: This is a plaintext resource referenced from a prompt`,
              },
            },
          },
        ],
      };
    }

    // Single PR validation handlers
    if (name === PromptName.VALIDATE_COMMUNITY_SERVER) {
      const pr_url = args?.pr_url as string;

      if (!pr_url) {
        throw new Error("pr_url is required");
      }

      const promptText = loadPromptTemplate("validate_community_server.md", {
        pr_url,
      });

      return {
        messages: [
          {
            role: "user",
            content: {
              type: "text",
              text: promptText,
            },
          },
          {
            role: "user",
            content: {
              type: "text",
              text: "Please validate PR: https://github.com/modelcontextprotocol/servers/pull/123",
            },
          },
          {
            role: "assistant",
            content: {
              type: "text",
              text: `- **PR Author**: developer123
- **Server Name**: awesome-mcp-server
- **Server URL**: https://github.com/developer123/awesome-mcp-server
- **Validation Status**: Valid
- **Confidence Level**: 95%
- **Validation Notes**: Server has clear documentation, proper MCP implementation, install instructions, and no security red flags. Repository is actively maintained with recent commits.`,
            },
          },
        ],
      };
    }

    if (name === PromptName.VALIDATE_OFFICIAL_SERVER) {
      const pr_url = args?.pr_url as string;

      if (!pr_url) {
        throw new Error("pr_url is required");
      }

      const promptText = loadPromptTemplate("validate_official_server.md", {
        pr_url,
      });

      return {
        messages: [
          {
            role: "user",
            content: {
              type: "text",
              text: promptText,
            },
          },
          {
            role: "user",
            content: {
              type: "text",
              text: "Please validate PR: https://github.com/modelcontextprotocol/servers/pull/456",
            },
          },
          {
            role: "assistant",
            content: {
              type: "text",
              text: `- **PR Author**: stripe-official
- **Server Name**: stripe-mcp-server
- **Server URL**: https://github.com/stripe/stripe-mcp-server
- **Validation Status**: Valid
- **Confidence Level**: 98%
- **Validation Notes**: Official Stripe repository verified. Organization is legitimate and matches the official Stripe GitHub org. Server has comprehensive documentation with installation instructions and usage examples. Icon URL (https://stripe.com/img/v3/home/social.png) is accessible and returns valid image. Code follows MCP protocol standards. No security concerns identified.`,
            },
          },
        ],
      };
    }

    throw new Error(`Unknown prompt: ${name}`);
  });
}
